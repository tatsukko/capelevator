...
public class Sim {
   ...
   public static void schedule(SimEvent event,double inter_time) {//(int event,double inter_time,TOKEN token)
	    ...
	    switch (event_list_type) {
                case Const.LINKED: insert_list(event_list,an_event,Const.TIME_KEY); break;
                case Const.HEAP: heap_insert(an_event); break;
            }
        }
    }
	...
	SimEvent empty_event = new SimEvent();
        ...
        if (current_node == null) {return_list.add(empty_event); return return_list;}
        
        while (exit_loop == Const.false_ && current_node != null) {
	  SimEvent an_event = new SimEvent();
	  switch(event_list_type) {
	   case Const.LINKED: status=list_read(event_list,an_event); 
	       if (status == -1) {return_list.add(empty_event); return return_list;} break;
           case Const.HEAP: status=heap_read(an_event); 
	       if (status == -1) {return_list.add(empty_event); return return_list;} break;
	  } // end of switch
          vtime = an_event.time;
	  ...
	  { 
		    ...
		    if (status == -1) {return_list.add(empty_event); return return_list;}
	...
	{
	   case Const.LINKED: status=list_read(event_list,an_event); 
	       if (status == -1) return empty_event; break;
           case Const.HEAP: status=heap_read(an_event); 
	       if (status == -1) return empty_event; break;
	}
        ...
        if(found==Const.true_) {
            ...
            if(facility[facility_id].queue.size > 0) {
                ...
                if (an_event.time < 0) {
                    ...
                    an_event.time = current_time;
                    switch (event_list_type) {
                        case Const.LINKED: add_front_list(event_list,an_event); break;
                        case Const.HEAP: heap_insert(an_event); break;
                    }
                }
            }
